fn main() {
    ...
    let backpacks : Vec<Vec<i32>> = puzzle_lines.split(|&elt| elt == "").map(|ss| ss.iter().filter_map(|&s|  {s.parse().ok()}).collect()).collect();
    println!("Part 1: {}.", backpacks.iter().map(|b| b.iter().sum::<i32>()).max().unwrap());
