fn main() {
    ...
    println!("Part 1: {}", pairs.iter().filter(|(r1, r2)| r2.contains(r1) || r1.contains(r2)).count());
    println!("Part 2: {}", pairs.iter().filter(|(r1, r2)| r1.overlaps(r2)).count());
...
fn parse_pairs(line: &str) -> (Range, Range) {
    ...
    (parse_range(&line[0 .. comma]), parse_range(&line[(comma + 1) .. (line.len())]))
}

fn parse_range(span: &str) -> Range {
    let dash = span.find(|c| c == '-').unwrap();
    Range{from: span[0 .. dash].parse().unwrap(), to: span[(dash + 1) .. (span.len())].parse().unwrap()}
}


#[test]
fn test_parse_pairs() {
    assert_eq!(parse_pairs("372-372,350-400"), (Range{from: 372, to: 372}, Range{from: 350, to: 400}));
}
